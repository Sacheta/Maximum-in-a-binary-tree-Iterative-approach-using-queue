//Program generating a whole binary tree at run time.To find the maximum in the given binary tree, it uses the iterative approach. It does a level order traversal using queue DS. It uses a spare variable sparen, to store ans update the value of maximum data value encountered till any given time
#include<iostream>
#include<climits>
#include<queue>
using namespace std;
...
int sparen;  //spare int variable

void node::maxn(node *root){  //iterative function for printing maximum in the tree using queue DS
    ...
    queue<node*>q;
    q.push(root);
    sparen=root->data;  //storing root's data sparen for comaprison
    while(!q.empty()){
        node *curr=q.front();
        q.pop();
        if(curr->data>sparen)  //if data of current node is greater than sparen, set sparen as the data of current node
            sparen=curr->data;
        if(curr->left!=nullptr)  //if left child of current node exists
            q.push(curr->left);
        if(curr->right!=nullptr)  //if right child of current node exists
            q.push(curr->right);
    }
    cout<<"Maximum is: "<<sparen<<endl;  //printing the value of sparen i.e., maximum of the tree
